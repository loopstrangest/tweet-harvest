...
export class WordCloudManager {
    ...
    checkWordCloudLibrary() {
        ...
        setTimeout(() => {
            if (typeof WordCloud === 'undefined') {
                console.warn('WordCloud2.js library not detected - will use fallback rendering');
            } else {
                console.log('WordCloud2.js library loaded successfully');
            }
        }, 1000);
    }
    ...
    async generateWordCloud(account, isAutoGenerated = false) {
        ...
        try {
            ...
            const allTweets = await this.fetchAllTweetsWithProgress(account, signal);
            ...
            // If this was auto-generated, provide visual feedback that it's ready
            if (isAutoGenerated) {
                // Update tab to show completion
                const wordCloudTab = document.querySelector('[data-tab="wordcloud"] i');
                if (wordCloudTab) {
                    wordCloudTab.className = 'fas fa-check-circle';
                    wordCloudTab.style.color = 'var(--spring-growth)';
                    
                    // Restore to cloud icon after 3 seconds
                    setTimeout(() => {
                        wordCloudTab.className = 'fas fa-cloud';
                        wordCloudTab.style.color = '';
                    }, 3000);
                }
            }
    ...
    async fetchAllTweetsWithProgress(account, signal = null) {
        ...
        while (true) {
            ...
            document.getElementById('progressText').textContent = 'Fetching tweets...';
    ...
    processTextForWordCloud(tweets) {
        ...
        const stopWords = new Set([
            ...
            'needn', 'shan', 'shouldn', 'wasn', 'weren', 'won', 'wouldn', 'rt', 'via', 'get', 'got',
            // Twitter specific
            'twitter', 'tweet', 'retweet', 'rt', 'via', 'http', 'https', 'www', 'com', 'co',
            'like', 'follow', 'following', 'followers', 'dm', 'pm', 'lol', 'lmao', 'omg', 'wtf',
            'fyi', 'btw', 'imo', 'imho', 'tho', 'gonna', 'wanna', 'gotta', 'kinda', 'sorta',
            'ya', 'yeah', 'yep', 'nah', 'nope', 'ok', 'okay', 'thanks', 'thank', 'thx', 'ty',
            // Single letters and short words that aren't meaningful
            'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'j', 'k', 'l', 'n', 'p', 'q', 'r', 'u', 'v', 'w', 'x', 'z',
            'ah', 'oh', 'eh', 'um', 'uh', 'hmm', 'hm', 'er', 'erm'
        ]);
        ...
        tweets.forEach(tweet => {
            ...
            const words = cleanText.split(/\s+/).filter(word => {
                return word.length > 2 && 
                       ...
                       !/^[a-z]{1,2}$/.test(word); // Skip 1-2 letter combinations
            });
    ...
    renderWordCloud(wordData) {
        ...
        console.log('WordData received:', wordData?.length, 'words');
        console.log('Sample words:', wordData?.slice(0, 3));
        console.log('WordCloud2 available?', typeof WordCloud !== 'undefined');
        ...
        const wordList = wordData.slice(0, 100).map(word => [word.text, word.size]);
        console.log('WordList for WordCloud2:', wordList.slice(0, 3));
        ...
        try {
            ...
            console.log('Calling WordCloud2.js...');
            ...
            WordCloud(canvas, {
                ...
                color: function() {
                    const colors = ['#004b23', '#006400', '#007200', '#008000', '#38b000', '#70e000', '#9ef01a'];
            ...
            console.log('WordCloud2 called successfully');
    ...
    renderFallbackWordCloud(ctx, wordData, displaySize, colors) {
        console.log('Using fallback word cloud rendering');
        ...
        const tryPlaceWord = (word, fontSize, color) => {
            ...
            for (let attempt = 0; attempt < attempts; attempt++) {
                ...
                const x = displaySize / 2 + Math.cos(angle) * radius - dimensions.width / 2;
                const y = displaySize / 2 + Math.sin(angle) * radius - dimensions.height / 2;
                ...
                // Check for overlaps
                const hasOverlap = placedWords.some(placed => isOverlapping(rect, placed));
        ...
        console.log(`Fallback rendering placed ${placedWords.length} words`);
    }
    
    renderFallbackExport(ctx, wordData, canvasSize, colors) {
        console.log('Using fallback export rendering');
        ...
        const tryPlaceWord = (word, fontSize, color) => {
            ...
            for (let attempt = 0; attempt < attempts; attempt++) {
                ...
                const x = canvasSize / 2 + Math.cos(angle) * radius - dimensions.width / 2;
                const y = canvasSize / 2 + Math.sin(angle) * radius - dimensions.height / 2;
        ...
        console.log(`Fallback export placed ${placedWords.length} words`);
    }
    ...
    async exportWordCloud(account) {
        ...
        try {
            // Create a high-quality 1000x1000 export canvas
            ...
            console.log('Creating export with word data:', this.currentWordData?.slice(0, 5));
            
            // Generate green color palette for export
            const colors = [
                '#004b23', '#006400', '#007200', '#008000', 
                '#38b000', '#70e000', '#9ef01a'
            ];
            
            // Convert word data to format expected by WordCloud2
            const wordList = this.currentWordData.map(word => [word.text, word.size]);
            console.log('Export word list:', wordList.slice(0, 5));
            
            // Simple export - just copy from display canvas if possible
            const displayCanvas = document.getElementById('wordCloudCanvas');
            if (displayCanvas) {
                console.log('Copying from display canvas for export');
                exportCtx.drawImage(displayCanvas, 0, 0, exportSize, exportSize);
            } else {
                console.log('No display canvas found, creating basic export');
                exportCtx.fillStyle = '#008000';
                exportCtx.font = 'bold 24px Arial';
                exportCtx.textAlign = 'center';
                exportCtx.fillText('Word Cloud Export', exportSize / 2, exportSize / 2);
            }

            // Create download link
            const link = document.createElement('a');
            link.download = `${account.username}_wordcloud_${exportSize}x${exportSize}.png`;
            ...
            console.log(`High-quality ${exportSize}x${exportSize} word cloud exported for @${account.username}`);
