...
export class EmojiManager {
  ...
  initializeEventListeners() {
    document
      .getElementById("exportEmojiChart")
      .addEventListener("click", () => this.exportChart());
  }
  ...
  async generateEmojiChart() {
    ...
    try {
      // Get tweets from centralized manager or fetch directly
      let tweets;
      if (this.tweetManager && this.tweetManager.getAllTweets().length > 0) {
        this.updateProgress("Using cached tweets...", 25);
        tweets = this.tweetManager.getTweetsForEmojis();
      } else {
  ...
  async fetchAllTweets() {
    ...
    while (hasMore) {
      ...
      if (batch && batch.length > 0) {
        ...
        this.updateProgress(
          `Loading tweets... (${tweets.length} loaded)`,
          progress
        );
      } else {
        hasMore = false;
  ...
  analyzeEmojis(tweets) {
    ...
    const emojiRegex =
      /[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]|[\u{1F900}-\u{1F9FF}]|[\u{1F018}-\u{1F270}]|[\u{238C}]|[\u{2194}-\u{21AA}]|[\u{23E9}-\u{23F3}]|[\u{23F8}-\u{23FA}]|[\u{24C2}]|[\u{25AA}-\u{25AB}]|[\u{25B6}]|[\u{25C0}]|[\u{25FB}-\u{25FE}]|[\u{2B05}-\u{2B07}]|[\u{2B1B}-\u{2B1C}]|[\u{2B50}]|[\u{2B55}]|[\u{3030}]|[\u{303D}]|[\u{3297}]|[\u{3299}]/gu;
    ...
    const chartHtml = `
            <div class="emoji-chart">
                <div class="chart-header">
                    ...
                    <p>Top ${emojiData.length} emojis from @${
                ...
                <div class="chart-bars">
                    ${emojiData
                      .map((item, index) => {
                        ...
                        return `
                            <div class="emoji-bar" style="animation-delay: ${
                              index * 0.05
                            }s;">
  ...
  exportChart() {
    ...
    try {
      ...
      ctx.fillText(
        `Most Used Emojis - @${this.currentAccount.username}`,
        canvas.width / 2,
        40
      );
